...
var webuploader = require('../../components/webuploader/src/webuploader');
...
class MaterialUpload extends React.Component {
	...
	setImgList(result) {
		...
		this.setState({
			...
			for (var i in _temp_fileQueued) {
				this.state.fileQueued[i].progressbar = $('#' + i + '-progressbar');
				this.state.fileQueued[i].uploadstate = $('#' + i + '-uploadstate');
				this.state.fileQueued[i].thumbview = $('#' + i + '-thumbview');
				this.state.fileQueued[i].wrapli = $('#' + i);
				...
				<div className={"material__wrap " + (tools.getLength(this.state.fileQueued) ? "show" : "")}
					...
					<div className="material__wrap-scroll" ref={o => this.material__wrap_scroll = o}>
						<ul className="clear">
							...
							{$.map(this.state.fileQueued, (item) => {
								return item.hide ? null : (
									<li id={item.fileid} className={"item " + (item.src ? "success" : "")} key={item.fileid}>
										<div className="material__item-inner material__item-inner-show-larg">
											...
											<div id={`${item.fileid}-upload-error`} className="material__upload-replay"
													 onClick={this.replayUploadError.bind(this, item)}>
												<div><img src={require('./icon/message_shoe.svg')}/></div>
												<div style={{marginTop: 10}}>上传失败<br/>点击重传</div>
											</div>

											<div className="material__hover-look-text" onClick={this.insertImage.bind(this, item)}>点击插入</div>
											...
											<div onClick={this.deleteMaterial.bind(this, item)} className="material__delete">
												<img src={require('./icon/close_show.svg')}/>
											</div>
	...
	componentDidMount() {
		...
		$('.material__wrap-scroll').hover(function () {
			var _this = $(this),
				height = _this.height(),
				_ul = _this.find('>ul'),
				_ul_height = _ul.height();

			$(window).on('mousewheel.preventDefault DOMMouseScroll.preventDefault', function (e) {
			...
			$(window).off('mousewheel.preventDefault DOMMouseScroll.preventDefault');
		});
	...
	createViewLarg() {
		...
		var right_error = $('#' + this.state.viewImgLargId + '-error');
		...
		body.on('mouseenter', '.material__item-inner-show-larg', function () {
			...
			var _loadfn = function () {
				...
				var h, w, scrollTop = $(window).scrollTop();
	...
	bindOnUploadEvent() {
		...
		uploader.on('fileQueued', function (file) {
			...
			_this.setState({
				...
				_this.state.fileQueued[file.id].progressbar = $('#' + file.id + '-progressbar');
				_this.state.fileQueued[file.id].uploadstate = $('#' + file.id + '-uploadstate');
				_this.state.fileQueued[file.id].thumbview = $('#' + file.id + '-thumbview');
				_this.state.fileQueued[file.id].wrapli = $('#' + file.id);
	...
	reinstateImage(src, img) {
		...
		$(this.material__wrap_scroll).length && $(this.material__wrap_scroll).scrollTop(999999);
