...
class Editor extends Component {
	...
	postArticle(event, displayorder, beforeback, callback) {
		...
		var _ploa = $.post(url, data, function (replayData) {
			_this.isSend = false;
	...
	render() {
		var height = $(window).height() - 60 - 60 - 30 - 150;
			...
			<form action={"/article/Preview"} method="post" target="_blank" id="preview__form">
				<div className="post__editor-page">
					<div className="content__header">
						<div className="content__header fixed">
							<div className="content__header-inner main clear">
								<div className="content__header-nav">
									<a href="/user/article.html">我的文章</a> > {blogid > 0 ? <a href="/user/article.html?draft=1">草稿箱</a> :
									<a href="javascript:;">写文章</a>}</div>
								<div className="content__header-right">
									<div className="content__header-btn preview">
										<input value={'预览'} className="preview__btn-submit" type="submit"/>
									</div>
		...
		if (!_this.state.editor) {
			_this.state.editor = UE.getEditor(_this.state.editor_id, {
				onready: function () {
					...
					var toolbarbox = $('#' + _this.state.editor_id).find('.edui-editor-toolbarbox');
					...
					var editor__tools_tips = $('#' + _this.state.editor_id + '-tools-tips');
			...
			<div id="${_this.state.editor_id}-tools-tips" class="editor__tools-tips-wrap">
				...
				<img class="editor__tools-tips-roow editor__tools-tips-roow2" src=${editor_roow2} />
						...
						editor__tools_tips = $('#' + _this.state.editor_id + '-tools-tips');
					...
					var __auto_save = $('#' + _this.state.editor_id + '-tools-save');
					...
					//hover编辑器图片出现效果
					var _temp_id_string = '', _temp_id_timer = null, doc = $('body', _this.state.editor.document);
	...
	autoSave() {
		...
		if (!this.previewBtn) {
			this.previewBtn = $('.preview,.post-article');
		}
