...
import webuploader from '../components/webuploader/dist/webuploader';
...
class Cover extends Component {
	...
	validation(_val,scroll) {
		if (_val && (!this.state.cover || !this.state.fileid)) {
			...
			if(scroll!='not' && this.content__cover){
				$('html,body').animate({
					scrollTop:$(this.content__cover).offset().top - 70
				});
			}
			return false;
		}
	...
	componentDidMount() {
		...
		var uploadstatusbar = $('.content__cover-uploadstate-bar');
		var content__cover = $('.content__cover');
